...
class IPSWApp:
	...
	def action_manage_downloads(cls, ipsw):
		...
		while running:
			...
			selection = term.menu("Showing firmwares in: " + ipsw.get_path(), choices, title=f"{APP_NAME} {APP_VERSION} - Firmware Manager", format_str="%s", format_fields=["file"], on_print_option=print_option, initial_index=initial_index, return_index=True, instructions=main_help, hotkeys=hotkeys_map, quit_keys=["x", "backspace", "esc"])
